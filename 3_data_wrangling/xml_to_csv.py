...
def shape_element(element, node_attr_fields=NODE_FIELDS, way_attr_fields=WAY_FIELDS,
    ...
    if element.tag == 'node':
        ...
        for tag in element.iter("tag"):
            ...
            if re.search(PROBLEMCHARS, k_value) == None:
                if re.search(LOWER_COLON, k_value):
                    node_tag['type'] = k_value.split(':',1)[0]
                    node_tag['key'] = k_value.split(':',1)[1]
                else:
                    node_tag['type'] = 'regular'
                    node_tag['key'] = k_value
        ...
        for tag in element.iter("tag"):
            ...
            if re.search(PROBLEMCHARS, k_value) == None:
                if re.search(LOWER_COLON, k_value):
                    way_tag['type'] = k_value.split(':',1)[0]
                    way_tag['key'] = k_value.split(':',1)[1]
                else:
...
#    process_map(OSM_PATH, validate=True) ## commented to prevent it from running and slowing the computer
